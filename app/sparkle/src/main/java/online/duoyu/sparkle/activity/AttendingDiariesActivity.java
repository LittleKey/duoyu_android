...
public class AttendingDiariesActivity extends SingleFragmentActivity {
  ...
  public void onEventMainThread(OnDiariesAmountUpdateEvent event) {
    if (event.apiType == ApiType.MY_ATTENDING_DIARIES) {
      updateTitle(SparkleUtils.formatString(R.string.all_attending_diaries, event.amount));
    }
  }
  ...
  protected BaseFragment createFragment(Intent intent) {
    ...
    bundle.putInt(Const.KEY_API_TYPE, ApiType.MY_ATTENDING_DIARIES.ordinal());
  ...
  protected String activityTitle() {
    ...
    return SparkleUtils.formatString(R.string.all_attending_diaries, model == null ? 0 : Wire.get(model.count.attentions, 0));
  }
}
